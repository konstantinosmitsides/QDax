...
class MapElitesRepertoire(flax.struct.PyTreeNode):
    ...
    def sample(
        ...
        Returns:
            ...
            return_samples: returns associated with sampled genotypes
            random_key: an updated jax PRNG random key
            trajectory_samples: trajectories associated with sampled genotypes
        """
    ...
    def add(
    ...
    ) -> Tuple[MapElitesRepertoire, jnp.ndarray]:
        """
        ...
        new_repertoire_trajectories = jax.tree_util.tree_map(
            lambda repertoire_trajectories, new_trajectories: (
                repertoire_trajectories.at[batch_of_indices.squeeze(axis=-1)].set(
                    new_trajectories
                )
            ),
            self.trajectories,
    ...
    def init(
        ...
        # Handle the case where extra_scores is None
        if extra_scores is not None:
            first_return = jax.tree_util.tree_map(
                lambda x: x[0], extra_scores["returns"]
            )
            first_trajectory = jax.tree_util.tree_map(
                lambda x: x[0], extra_scores["transitions"]
            )
        else:
            # Provide default values if extra_scores is None
            first_return = None
    ...
    def init_default(
        ...
        Args:
            ...
            returns: the returns to be used as a template
            trajectories: the trajectories to be used as a template
        ...
        default_returns = jax.tree_util.tree_map(
            lambda x: (
                jnp.zeros(shape=(num_centroids,) + x.shape, dtype=x.dtype)
                if x is not None
                else None
            ),
            returns,
